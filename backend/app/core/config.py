...
class Settings:
    ...
    # --- ADDED: AWS Credentials for DynamoDB ---
    ...
    def __init__(self):
        ...
        if not self.SECRET_KEY:
            raise ValueError("❌ CRITICAL ERROR: SECRET_KEY is missing!")
        if not self.FIREBASE_CREDENTIALS_PATH:
            raise ValueError("❌ CRITICAL ERROR: FIREBASE_CREDENTIALS_PATH is missing!")
        ...
        if not self._CORS_ORIGINS_RAW:
            raise ValueError("❌ CRITICAL ERROR: BACKEND_CORS_ORIGINS is missing!")
        
        try:
            self.BACKEND_CORS_ORIGINS = json.loads(self._CORS_ORIGINS_RAW)
        except json.JSONDecodeError:
            raise ValueError("❌ CRITICAL ERROR: BACKEND_CORS_ORIGINS is not valid JSON!")
